...
let seq_to_string s =
  ...
  Printf.sprintf "seq %s := %s to %s%s" s.seqheader_var
    ...
    (match s.seqheader_increase with
       None -> ""
     | Some e -> " by " ^ (exp_to_string e))

let to_string = function
    VDecl varDecl -> CaoPrinter.vardecl_to_string varDecl
  | CDecl const_d -> CaoPrinter.const_d_to_string const_d
  | Assign assign -> CaoPrinter.assign_to_string assign
  | Sample lVals -> CaoPrinter.sample_to_string lVals
  | FCallS fcalls -> CaoPrinter.fcalls_to_string fcalls
  | Ret expr -> "return " ^ CaoPrinter.exp_to_string expr
  | Ite expr -> Printf.sprintf "if (%s)" (CaoPrinter.exp_to_string expr)
  | Seq seq -> seq_to_string seq
  | While expr -> Printf.sprintf "while (%s)" (CaoPrinter.exp_to_string expr)
...
let flow f = let open Set.Infix in let open Ast in
  ...
  and flow_block acc = function
    ...
    | s1 :: s2 :: b -> let init_s2 = init s2 in
        ...
        flow_block (flow_rec (flow_rec (Set.union acc (Set.map (Utils.rev_pair init_s2) final_s1)) s1) s2) (s2 :: b)
