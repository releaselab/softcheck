...
and polinomial_to_string pol =
  Utils.sprint_list ~first:"" ~last:"" ~sep:"" monomial_to_string pol.monomials
...
and exp_to_string = function
  ...
  | FunCall fc -> Utils.sprint_list ~first:(fc.funcall_id ^ "(") ~last:")"
                    ~sep:", " exp_to_string fc.funcall_args
...
let vardecl_to_string vd =
  let ids,typedecl,init = match vd with
      VarD var_d ->
        let ids = Utils.sprint_list identity ~sep:"," var_d.var_d_ids in
...
let assign_to_string a =
  let ids = Utils.sprint_list ~sep:"," lval_to_string a.assign_ids in
  let exprs = Utils.sprint_list ~sep:"," exp_to_string a.assign_values in
  sprintf "%s := %s" ids exprs

let sample_to_string l =
  let ids = Utils.sprint_list ~sep:"," lval_to_string l in
  ids ^ " := $"

let fcalls_to_string f =
  let id = f.fcalls_id in
  let args = Utils.sprint_list ~sep:"," exp_to_string f.fcalls_args in
  sprintf "%s(%s)" id args
