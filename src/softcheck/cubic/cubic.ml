...
module Make(V : VAR)(T : TOKEN) = struct
  ...
  let get_token_int t = try
      Hashtbl.find tokens_ids t
    with Not_found ->
  ...
  module G = struct
    ...
    module D = Graph.Path.Dijkstra(NGraph)(struct
  ...
  let rec add_and_propagate_bits s v =
    ...
    if new_tokens <> old then
      ...
      let () = Bitv.iteri (fun i _ ->
        let conditionals = Hashtbl.find_default n.conditionals i [] in
        List.iter (fun (v1,v2) -> add_constraint (Subset (v1,v2))) conditionals) diff in
  ...
  and add_constraint = function
    ...
    | Conditional ((t,x),(y,z)) ->
        ...
        if Bitv.get n_x.token_sol (get_token_int t) then
          ...
          let conds = Hashtbl.find_or_add n_x.conditionals (get_token_int t) [] in
  ...
  let get_solution () =
    ...
    Hashtbl.map (fun v _ ->
      Bitv.foldi_left (fun acc i t ->
        if t then Hashtbl.find id_to_token i :: acc else acc) [] (get_or_put_vertex v |> G.label).token_sol) vars_vertex
end
