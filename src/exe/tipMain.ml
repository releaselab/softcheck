...
module Parser = ParsingUtils.Make(TipAst)(TipParser)(TipLexer)
module Flow = Tip.Flow
module Cfg = Tip.Cfg
module InterCfg = Tip.InterCfg
...
let ae p =
  ...
  let module AvailableExpressions = AvailableExpressionsTip.Solve(struct let p = p end) in
...
let rd p =
  ...
  let module ReachingDefinitions = ReachingDefinitionsTip.Solve(struct let p = p end) in
...
let vb p =
  ...
  let module VeryBusyExpressions = VeryBusyExpressionsTip.Solve(struct let p = p end) in
...
let lv p =
  ...
  let module LiveVariables = LiveVariablesTip.Solve(struct let p = p end) in
...
let sa p =
  let open Printf in
  print_endline "Sign";
  let module SignAnalysis = SignAnalysisTip.Solve(struct let p = p end) in
...
let ta p =
  ...
  let module TaintAnalysis = TaintTip.Solve(struct let p = p end) in
  ...
  let aux (l,e) =
    ...
    let rec aux_rec = function
      ...
      | Eident i -> match Map.find i result with
          Taint_lattice.Element true -> printf "Tainted information reaches sink in %d\n" l
        | Taint_lattice.Top -> printf "Tainted information may reach sink in %d\n" l
        | _ -> ()
    in aux_rec e in
  List.iter aux sinks
